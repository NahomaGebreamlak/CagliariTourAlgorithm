...
def update_state(previous_state, new_distribution, decay=0.5):
    updated_interests = [
        (decay * p + (1 - decay) * n)
        for p, n in zip(previous_state[2], new_distribution)
    ]
...
def choose_action(state, epsilon):
    ...
    state_actions = QValue.objects.filter(
        age=age, nationality=nationality, interests=interests
    )
...
def q_learning_update(state, action, reward, next_state):
    ...
    next_state_actions = QValue.objects.filter(
        age=next_state[0], nationality=next_state[1], interests=next_state[2]
    )
...
def haversine_distance(loc1, loc2, public_transport_percentage=0, taxi=False):
    ...
    if taxi:
        ...
        walking_distance = (
            distance * walking_percentage * 1.2
        )  # Increase by 20% for walking
        # Adjust distance for public transport (public transport can be less direct)
        public_transport_distance = (
            distance * public_transport_percentage * 1.1
        )  # Increase by 10% for public transport
...
def parse_time(time_str):
    ...
    try:
        time_obj = datetime.strptime(
            time_str.strip(), "%I:%M%p" if ":" in time_str else "%I%p"
        )
...
def fitness(
    ...
    open_route = [
        place
        for place, time_slot in zip(route, time_slots[: len(route)])
    ...
    for place in open_route:
        ...
        num_ratings = (
            place.user_rating_accessibility
            if isinstance(place.user_rating_accessibility, (int, float))
            and place.user_rating_accessibility > 0
            else 1
        )
        ...
        composite_score = (0.6 * normalized_rating + 0.4 * positive_ratio) * (
            1 + num_ratings
        )
...
def calculate_route(request, numberofdays):
    try:
        ...
        nationality = (
            request.GET.get("race") or "DefaultRace"
        )  # Replace "DefaultRace" with your default race

        num_days = int(numberofdays)

        public_transport = float(
            request.GET.get("public_transport", 0)
        )  # Default to 0 if not provided
        taxi = (
            request.GET.get("taxi", "false").lower() == "true"
        ...
        if len(all_places) < population_size:
            logger.warning(
                "Population size exceeds available places. Adjusting population size."
            )
        ...
        for day in range(num_days):
            ...
            if len(population) == 0:
                ...
                return JsonResponse(
                    {"error": f"No valid routes available for day {day}."}, status=400
                )
            ...
            for gen in range(generations):
                if len(population) < 2:
                    ...
                    return JsonResponse(
                        {"error": "Insufficient population for crossover."}, status=400
                    )
                ...
                population = sorted(
                    ...
                    key=lambda x: fitness(
                        x, action, public_transport, taxi, time_slots
                    ),
                    reverse=True,
                )[:population_size]
...
def calculate_feedback_distribution(feedback_pois):
    ...
    }  # Start with zero for each category
    ...
    if total_pois == 0:
        return [1 / len(categories)] * len(
            categories
        )  # Default to equal distribution if no POIs
...
def feedback_route(request):
    ...
    try:
        ...
        if not guide_data or not isinstance(guide_data, list):
            return JsonResponse(
                {"error": "Invalid feedback guide structure."}, status=400
            )
        ...
        for day_entry in guide_data:
            ...
            updated_state = update_state(
                session_state, feedback_distribution, decay=0.5
            )
...
def print_unique_categories(request):
    ...
    try:
        ...
        return JsonResponse(
            {"error": f"Could not retrieve unique categories: {str(e)}"}, status=500
        )
...
def get_route_and_graph(request, numberofdays):
    try:
        ...
        for _ in range(50):
            ...
            # Parse the response JSON and process POIs
            result = response.json()
            for guide in result["guide"]:  # Iterate through each guide
                for poi in guide["POIs"]:  # Iterate through POIs of each guide
                    all_places.append(poi)

        # Aggregate place frequencies across all days for the graph
        place_counts = pd.Series(all_places).value_counts()

        # Truncate place names to 8 characters for consistent display
        truncated_place_names = [
            place[:8] + "..." if len(place) > 8 else place
            for place in place_counts.index
        ]
        place_counts.index = (
            truncated_place_names  # Update the place names with truncated versions
        )

        # Prepare a dictionary for the places and their counts
        place_counts_dict = place_counts.to_dict()

        # Plot the graph (Place Selection Frequency)
        fig, ax = plt.subplots(
            figsize=(14, 8)
        )  # Increased figure size for better readability
        place_counts.sort_values(ascending=False).plot(
        ...
        )  # Rotate labels, larger font for readability

        # Save the graph
        graph_path = os.path.join(
            "static", "graphs", "place_frequency_with_rotated_labels.png"
        )
        ...
        # Render the template with the path to the graph and the table data
        return render(
            request,
            "cagliaritour/graphs.html",
            {
